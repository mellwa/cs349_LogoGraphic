...
// TODO CS349
      ...
      for (vector<Component*>::iterator iter=this->children.begin(); iter != this->children.end(); iter++){
          Component* c = (*iter);
          if(c->GetName() == TURTLE_GRAPHICS_WIDGET_ID) continue;
          if(c->IsVisible()){
  ...
  void Component::Paint(Graphics* g)
  {
    ...
    if (this->GetBackgroundColor() == this->GetForegroundColor()) {
      ...
      if(!children.empty()){
          for (vector<Component*>::iterator child=children.begin(); child != children.end(); child++) {
              g->SetClipRect(this->GetBounds());
              (*child)->Paint(g);
          }
      }
  ...
  void Component::SetBounds(const Rectangle & r)
  {
      Rectangle tmpR;
      if(this->GetParent() != NULL){
          tmpR.x = r.x+this->GetParent()->GetBounds().x;
          tmpR.y = r.y+this->GetParent()->GetBounds().y;
          tmpR.width = r.width;
